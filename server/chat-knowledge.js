...
- Paiement : tous les paiements de commandes se font uniquement à la livraison du colis, via iKeePay, le partenaire de paiement officiel de Mboppi (Mobile Money uniquement : Orange Money, MTN Mobile Money, Wave, Moov Money, M-Pesa, etc. — aucune carte bancaire n'est jamais demandée ni acceptée). Le livreur déclenche la demande de paiement sur le numéro Mobile Money du client au moment de la livraison. Aucun autre moyen de paiement n'est proposé actuellement (ni espèces, ni virement manuel). La part iKeePay (6 %) est incluse dans le montant demandé : le client paie le total majoré des frais, la boutique et les vendeurs reçoivent leurs montants nets. Seuls l'activation du compte vendeur et les dons se paient en ligne immédiatement (même prestataire iKeePay).
...
- Commissions : la boutique fixe un pourcentage de commission par produit (affiché sur la fiche produit). Le vendeur reçoit cette commission pour chaque vente réalisée grâce à lui. Le parrainage rapporte 2 % : quand un CLIENT s'inscrit avec le code vendeur d'un vendeur, il devient son client affilié et ses achats rapportent 2 % de leur montant à ce vendeur parrain (versé automatiquement dès 1500 F cumulés). Tous les reversements sont nets (frais iKeePay de 6 % déduits).
- Vendeur : l'espace vendeur est payant — activation de 31 jours (1500 F) payable en ligne via iKeePay depuis la page de paiement.
...
- Payment: all order payments are made only on delivery of the package, via iKeePay, Mboppi's official payment partner (mobile money only: Orange Money, MTN Mobile Money, Wave, Moov Money, M-Pesa, etc. — a credit card is never requested or accepted). The delivery rider triggers the payment request to the customer's mobile money number at delivery. No other payment method is currently offered (no cash, no manual transfer). The iKeePay share (6%) is included in the requested amount: the customer pays the total plus fees, and shops and sellers receive their net amounts. Only seller account activation and donations are paid online immediately (same iKeePay provider).
...
- الدفع: جميع دفعات الطلبات تتم فقط عند تسليم الطرد عبر iKeePay، شريك الدفع الرسمي لمبوّي (المال المحمول فقط: أورنج موني، إم تي إن موبايل موني، ويف، موف موني، إم بيسا... — لا تُطلب بطاقة بنكية أبداً ولا تُقبل). يطلق الموصّل طلب الدفع إلى رقم المال المحمول للعميل عند التسليم. لا يُقدَّم حالياً أي وسيلة دفع أخرى (لا نقداً ولا تحويلاً يدوياً). حصة iKeePay (6٪) مشمولة في المبلغ المطلوب: يدفع العميل الإجمالي بالإضافة إلى الرسوم، وتتلقى المتاجر والباعة مبالغهم صافية. فقط تفعيل حساب البائع والتبرعات تُدفع عبر الإنترنت فوراً (نفس مزود iKeePay).
...
- العمولات: يحدد المتجر نسبة عمولة لكل منتج (تظهر في صفحة المنتج). يحصل البائع على هذه العمولة عن كل بيع تم بفضله. الإحالة تمنح 2٪: عندما يسجّل عميل برمز البائع، يصبح عميلاً تابعاً له، وتدرّ مشترياته 2٪ من قيمتها على البائع المُحيل (تُدفع تلقائياً عند بلوغ 1500 ف. مجمّعة). جميع التحويلات صافية (تُخصم رسوم iKeePay بنسبة 6٪).
- البائع: مساحة البائع مدفوعة — تفعيل لمدة 31 يوماً (1500 ف. فرنك) يُدفع عبر الإنترنت بواسطة iKeePay من صفحة الدفع.
