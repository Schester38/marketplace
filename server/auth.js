...
export function signToken(user) {
  return jwt.sign(
    { id: user.id, email: user.email, role: user.role, name: user.name, verified: Boolean(user.verified), membership_expires_at: user.membership_expires_at || null },
    SECRET,
    { expiresIn: '24h' }
  );
}
...
export function roleRequired(...roles) {
  return async (req, res, next) => {
    ...
    if (['shop', 'seller'].includes(req.user.role)) {
      const current = (await q('SELECT verified, membership_expires_at FROM users WHERE id = $1', [req.user.id]))[0];
      if (current && !current.verified && (!current.membership_expires_at || new Date(current.membership_expires_at) <= new Date())) {
        return res.status(402).json({ error: 'Adhésion requise pour accéder à cet espace', code: 'MEMBERSHIP_REQUIRED' });
      }
    }
    next();
  };
