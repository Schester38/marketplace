  ...
  try {
    ...
    if (!html) {
      return res
        ...
        .type("html")
        .send(
          `<!doctype html><html lang="fr"><head><meta charset="UTF-8"/><title>${title.replace(/</g, "")}</title>${Object.entries(
            {
              description: descText,
              "og:title": title,
            ...
            .map(([k, v]) =>
              k.startsWith("og:")
                ? `<meta property="${k}" content="${v.replace(/"/g, "&quot;")}"/>`
...
router.get("/sitemap.xml", async (req, res) => {
  try {
    ...
    const entries = [
      ...
      ...CITIES.map(([slug]) => ({
        loc: `${BASE_URL}/ville/${slug}`,
        freq: "weekly",
        prio: "0.7",
      })),
      ...offers.map((o) => ({
        loc: `${BASE_URL}/offre/${o.id}`,
        freq: "weekly",
        prio: "0.7",
        lastmod: o.created_at,
      })),
      ...products.map((p) => ({
        loc: `${BASE_URL}/produit/${p.id}`,
        freq: "weekly",
        prio: "0.8",
  ...
  try {
    ...
    const jsonLd = {
      ...
      offers: {
        ...
        availability:
          Number(o.quantity) > 0 ? "https://schema.org/InStock" : "https://schema.org/OutOfStock",
        url: canonical,
      },
    };
