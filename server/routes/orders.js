...
router.post("/", optionalAuth, async (req, res, next) => {
  try {
    ...
    const result = await withTransaction(async (tx) => {
      ...
      for (const it of items) {
        ...
        if (!Number.isInteger(pid) || !Number.isInteger(qty) || qty < 1) {
          const e = new Error("Article invalide");
          e.statusCode = 400;
          throw e;
        }
        ...
        if (req.user) {
          const b = (
            await tx.query("SELECT referred_by FROM users WHERE id = $1", [req.user.id])
          )[0];
      ...
      for (const s of createdSales) {
        await tx.query(
          `INSERT INTO notifications (user_id, type, sale_id) VALUES ($1, 'sale_order', $2)`,
          [s.shop_id, s.id]
        );
        if (s.referred_by)
          await tx.query(
    ...
    for (const s of result.createdSales) {
      await sendPush(s.shop_id, {
        title: "Nouvelle commande 🛒",
        body: `${s.name} ×${s.quantity} — ${String(buyer_name).trim()}.`,
        url: "/shop",
      });
      if (s.referred_by)
        await sendPush(s.referred_by, {
          title: "Votre filleul a commandé 🎁",
          body: `${String(buyer_name).trim()} a commandé « ${s.name} » — vous recevrez 2% (${s.referralCommission} F) après livraison.`,
          url: "/seller",
        });
    }
    const sales = await q(
...
async function uniqueConfirmCodeTx(tx) {
  for (let i = 0; i < 20; i++) {
    const c = randomCode(6);
    const exists = (
      await tx.query("SELECT id FROM sales WHERE confirm_code = $1 FOR SHARE", [c])
    )[0];
    if (!exists) return c;
  }
...
router.get("/me", authRequired, async (req, res) => {
  const orders = await q("SELECT * FROM orders WHERE user_id = $1 ORDER BY created_at DESC", [
    req.user.id,
  ]);
