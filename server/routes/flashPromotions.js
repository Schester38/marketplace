...
function promoRow(row) {
  ...
  return {
    ...
    discount_percent:
      row.price > 0 ? Math.round((1 - Number(row.promo_price) / Number(row.price)) * 100) : 0,
...
router.post(
  ...
  authRequired,
  roleRequired("shop"),
  validate(createFlashPromoSchema),
  ah(async (req, res) => {
    const { product_id, promo_price, duration_minutes } = req.body;
    const product = (
      await q(
        "SELECT id, price, quantity, name, currency FROM products WHERE id = $1 AND shop_id = $2",
        [Number(product_id), req.user.id]
      )
    )[0];
    if (!product) {
    ...
    if (price >= Number(product.price)) {
      return res
        .status(400)
        .json({ error: "Le prix promotionnel doit être inférieur au prix normal." });
    }
    const minutes = Math.max(
      1,
      Math.min(MAX_MINUTES, Math.round(Number(duration_minutes) || DEFAULT_MINUTES))
    );
    const conflict = (
      await q(
    ...
    if (recent >= MAX_WEEKLY_PER_SHOP) {
      return res
        .status(400)
        .json({ error: "Vous ne pouvez publier qu'une seule promotion par semaine." });
    }
    const created = (
      await q(
