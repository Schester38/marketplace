...
router.post('/', authRequired, roleRequired('seller'), ah(async (req, res) => {
  ...
  const result = await withTransaction(async (tx) => {
    ...
    // Une promotion éclair cache le produit : il n'est plus vendable par les vendeurs tant qu'elle dure.
    const promo = (await tx.query(
      'SELECT 1 FROM flash_promotions WHERE product_id = $1 AND ends_at > now()',
      [product.id]
    ))[0];
    if (promo) {
      const error = new Error('Ce produit est en promotion et n\'est pas disponible à la vente par les vendeurs pour le moment.');
