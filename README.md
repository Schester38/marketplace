...
- **Créateur** : publie des créations (catégorie Arts & Artisanat) et gère son espace.
...
- **Avis et notations** : les clients notent 1 à 5 étoiles les produits reçus, avec distribution et commentaires.
- **Chat IA** : assistant virtuel Gemini multilingue (fr/en/ar) qui injecte le catalogue en temps réel.
- **Notifications push** : Web Push VAPID pour les commandes, livraisons, commissions et parrainages.
- **Newsletter** : inscription publique, désabonnement par token, envoi admin.
- **Vitrine d'offres (Verone)** : offres publiques sans compte, avec partage et présentation HTML.
- **Facture PDF** : génération automatique pour les ventes livrées.
- **Export des ventes** : CSV/JSON pour les espaces vendeur, boutique et créateur.
- **Favoris** : page dédiée pour les clients.
- **Mode lite** : allègement de l'UI.
- **PWA** : installation, cache et page hors-ligne.
...
Les boutiques et vendeurs doivent aussi régler une adhésion via Ikeepay de 30 jours : 2 500 XAF pour une boutique et 1 500 XAF pour un vendeur. Un compte validé par l'administrateur peut accéder à son espace sans paiement. Un vendeur parrain reçoit **1 000 XAF** lorsqu'un vendeur inscrit avec son code paie son adhésion ; les **500 XAF** restants sont reversés au wallet plateforme Mboppi.
...
- [server/routes/payments.js](server/routes/payments.js) expose les endpoints Ikeepay (membership, payin, webhook).
...
| POST | `/api/products/:id/duplicate` | shop, creator | Dupliquer un produit |
...
| POST | `/api/sales/grouped-pay` | shop | Paiement groupé vendeur/parrain |
...
| POST | `/api/payments/ikeepay/payin` | public | Initier un paiement Ikeepay |
| POST | `/api/payments/ikeepay/membership` | auth | Initier une adhésion Ikeepay |
| POST | `/api/payments/ikeepay/webhook` | public | Webhook Ikeepay |
