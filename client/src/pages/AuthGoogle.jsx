...
import { requestPushPermission, armPushPromptOnFirstInteraction } from "../push.js";
...
export default function AuthGoogle() {
  ...
  const pickSpace = async (role) => {
    ...
    try {
      ...
      armPushPromptOnFirstInteraction();
  ...
  useEffect(() => {
    ...
    Promise.race([api.me(), timeout])
      .then((data) => {
        ...
        // Déjà autorisé → réinscription silencieuse (idempotent). Sinon on
        // arme la popup native sur le PREMIER clic de l'utilisateur (le geste
        // du clic Google initial a été perdu dans la redirection) ; la
        // bannière NotificationPrompt reste le filet de secours.
        requestPushPermission();
        armPushPromptOnFirstInteraction();
        storage.setItem("mboppi_welcome", "login");
        navigate(postLoginPath(data.user), { replace: true });
