...
export default function SellerPayments() {
  ...
  const save = async (e) => {
    ...
    try {
      ...
      // Moyen principal : au plus un wallet `primary` ; si aucun n'est coché
      // comme principal, le premier devient le principal (rétrocompat).
      let seen = false;
      ...
      const d = await api.updatePaymentMethods({ full_name: fullName, wallets: safe });
      ...
      <section className="dash-header">
        <div>
          ...
          <p className="hint" style={{ marginTop: 8 }}>
            {t(
              "Les paiements automatiques sont traités via Ikeepay. Ikeepay prélève 10% de frais de traitement sur chaque transaction. Les montants que vous recevez correspondent aux commissions calculées par Mboppi, nettes des frais Ikeepay."
            )}
          </p>
        </div>
      </section>
