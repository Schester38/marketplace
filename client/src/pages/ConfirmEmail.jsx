      ...
      <div className="card form-card" style={{ textAlign: 'center' }}>
        ...
        {state === 'loading' && (
          <>
            ...
            <p className="hint" style={{ textAlign: 'center' }}>{t('Nous vérifions votre adresse email…')}</p>
          </>
        )}
        {state === 'ok' && (
          <>
            ...
            <Link to="/login" className="btn btn-primary btn-block">{t('Se connecter')}</Link>
          </>
        )}
        {state === 'error' && (
          <>
            ...
            {message && <p className="hint" style={{ textAlign: 'center', marginTop: 10 }}>{message}</p>}
            <Link to="/login" className="btn btn-outline btn-block">{t('Aller à la connexion')}</Link>
          </>
        )}
      </div>
    </main>
