  ...
  return (
    <main className="container">
      ...
      <section className="hero city-hero">
        ...
        <p>
          {t(
            "Découvrez les boutiques et les créations disponibles à {city}. Commandez en ligne ou par WhatsApp et recevez chez vous.",
            { city }
          )}
        </p>
        <div className="city-links">
          <Link to="/register" className="btn btn-primary">
            {t("Ouvrir ma boutique")}
          </Link>
        </div>
      </section>
      ...
      <section className="section" aria-label={t("Produits à {city}", { city })}>
        <div className="section-head">
          <h2 className="section-title">
            <Logo className="logo-inline" /> {t("Produits disponibles à {city}", { city })}
          </h2>
        </div>
        ...
        {products === null ? (
          <div className="grid">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="card page-center">
                <div className="skeleton-block" style={{ height: 180 }}></div>
              </div>
            ))}
          </div>
        ) : products.length === 0 ? (
          <div className="card page-center">
            <p className="empty">
              {t("Aucun produit publié pour le moment à {city}. Revenez bientôt !", { city })}
            </p>
