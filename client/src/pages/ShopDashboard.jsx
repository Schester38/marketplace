...
export default function ShopDashboard() {
  ...
  useEffect(() => {
    api
      .shopCode()
      .then((d) => setShopCode(d.shop_code))
      .catch(() => {});
  }, []);
  ...
  const submitProduct = async (e) => {
    ...
    if (
      ...
      (oldNum !== null && !Number.isFinite(oldNum)) ||
      (oldNum !== null && oldNum < 0)
    ) {
      setError(t("Prix invalide"));
      return;
    }
    // --- RÔLE BOUTIQUE : produits PHYSIQUES uniquement ---------------------
    // Un compte boutique ne publie jamais de produit digital (fichier) : cette
    // responsabilité appartient aux comptes CRÉATEUR. Le serveur fait autorité
    // (403 si un fichier `digital` est envoyé par une boutique) — aucun champ
    // digital n'est transmis ici.
    const payload = {
      ...form,
  ...
  const duplicateProduct = async (id) => {
    ...
    try {
      ...
      setSuccess(
        t("Produit « {name} » dupliqué. Pensez à modifier la copie.", { name: product.name })
      );
      load();
    } catch (err) {
      setError(err.message);
  ...
  const removeDelivered = async (s) => {
    if (
      !window.confirm(
        t("Supprimer cette livraison « {name} » de mon espace ?", { name: s.product_name })
      )
    )
      return;
    setError("");
  ...
  const openPayGrouped = async (kind, group) => {
    ...
    try {
      ...
      const d = first
        ? await api.salePaymentMethods(first.id, kind === "referral" ? "referral" : null)
        : { methods: null };
  ...
  const submitPay = async (e) => {
    ...
    try {
      if (payForm.target === "grouped") {
        const d = await api.groupedPay(
          payForm.kind,
          payForm.group.seller_id || payForm.group.parrain_id,
          payForm.proof
        );
        setSuccess(
          payForm.kind === "referral"
            ? t("Parrain payé ! La preuve a été enregistrée.")
            : t("Vendeur payé ! La preuve a été enregistrée.")
        );
        setPayForm(null);
        load();
        return;
    ...
    <main className="container">
      ...
      <section className="dash-header">
        ...
        <div className="row2 dash-actions">
          ...
          >
            🚚 {t("Commandes livrées")}{" "}
            {deliveredSales.length > 0 ? `(${deliveredSales.length})` : ""}
          </button>
          <button
      ...
      <div className="dash-shell">
        ...
        <div className="dash-main">
            ...
            <section className="kpi-grid" aria-label={t("Indicateurs clés")}>
              <div className="kpi-card">
                ...
                <span className="kpi-body">
                  <span className="kpi-value">{stats.total_sales}</span>
                  <span className="kpi-label">{t("Ventes enregistrées")}</span>
                </span>
              </div>
              <div className="kpi-card">
                <span className="kpi-icon ki-green" aria-hidden="true">
                ...
                <span className="kpi-body">
                  ...
                  <span className="kpi-label">{t("Chiffre d'affaires")}</span>
                </span>
              </div>
              <div className="kpi-card">
                <span className="kpi-icon ki-violet" aria-hidden="true">
                  <IconTruck size={20} />
                </span>
                <span className="kpi-body">
                  <span className="kpi-value">
                    {formatMoney(stats.delivery_revenue)} {symbol}
                  </span>
                  <span className="kpi-label">{t("Livraisons")}</span>
                </span>
              </div>
              <div className={`kpi-card${Number(stats.owed_commission) > 0 ? " kpi-danger" : ""}`}>
                <span className="kpi-icon ki-orange" aria-hidden="true">
                  <IconBanknote size={20} />
                </span>
                <span className="kpi-body">
                  <span className="kpi-value">
                    {formatMoney(stats.owed_commission)} {symbol}
                  </span>
                  <span className="kpi-label">{t("Commissions à verser")}</span>
                </span>
              </div>
              <div className="kpi-card">
                <span className="kpi-icon ki-green" aria-hidden="true">
                ...
                <span className="kpi-body">
                  <span className="kpi-value">
                    {formatMoney(stats.paid_commission)} {symbol}
                  </span>
              ...
              <div className="grid">
                {flashPromos.map((pr) => (
                  <FlashPromoCard
                    key={pr.id}
                    promo={pr}
                    onDelete={removeFlash}
                    onShare={shareFlashPromo}
                    showShop={false}
                  />
                ))}
              </div>
              ...
              {(() => {
                ...
                for (const s of deliveredSales) {
                  ...
                  if (s.referred_by) {
                    ...
                    if (!referralGroups.has(key))
                      referralGroups.set(key, {
                        parrain_id: s.referred_by,
                        parrain_name: s.parrain_name || "—",
                        items: [],
                        pending: 0,
                        anyClaimed: false,
                      });
                ...
                const rgs = [...referralGroups.values()]
                  .filter((g) => g.pending > 0)
                  .sort((a, b) => b.pending - a.pending);
                      ...
                      {deliveredSales.map((s) => {
                        ...
                        return (
                          <tr key={s.id}>
                            ...
                            <td>
                              {formatMoney(
                                Number(s.total_price || 0) + Number(s.delivery_fee || 0)
                              )}{" "}
                              {countrySymbol(s.shop_country)}
                            </td>
                            <td>
                            ...
                            <td>
                              <div className="row2" style={{ justifyContent: "flex-end", gap: 6 }}>
                                ...
                                >
                                  🗑️{" "}
                                  {pendingCommission ? t("Commission non payée") : t("Supprimer")}
                                </button>
                              </div>
                            </td>
                          </tr>
          ...
          {showForm && (
            <div className="card form-card">
              ...
              <div className="photo-tip">
                ...
                <ol>
                  <li>
                    {t("Téléchargez l'application ChatGPT si vous ne l'avez pas encore.")}
                  </li>
                  <li>
              ...
              <form onSubmit={submitProduct}>
                ...
                <div className="row2">
                  <div>
                    ...
                    <input
                      className="input"
                      placeholder={t("ex : 6 mois, 1 an, 2 ans")}
                      value={form.warranty}
                ...
                <div className="row2">
                  <div>
                    ...
                    <input
                      ...
                      min="0"
                      step="0.01"
                      placeholder={t("ex : 5000 (s'affiche barré)")}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min="0"
                      step="0.01"
                      placeholder={t("ex : 3500 (s'affiche en vert)")}
                      value={form.price}
          ...
          {products.length === 0 ? (
            <p className="empty">
              {t("Aucun produit pour le moment. Ajoutez votre premier produit (max 5).")}
            </p>
          ) : (
          ...
          <section className="card stats">
            ...
            {stats ? (
              <div className="stats-row">
                ...
                <div>
                  <span className="label">{t("Livraisons")}</span>
                  <strong>
                    {formatMoney(stats.delivery_revenue)} {symbol}
                  </strong>
                </div>
                <div>
                  <span className="label">{t("Commissions à verser")}</span>
                  <strong className={stats.owed_commission > 0 ? "text-danger" : ""}>
                    {formatMoney(stats.owed_commission)} {symbol}
                  </strong>
                </div>
                <div>
                  <span className="label">{t("Commissions versées")}</span>
                  <strong>
                    {formatMoney(stats.paid_commission)} {symbol}
                  </strong>
                </div>
                    ...
                    {activeSales.map((s) => {
                      ...
                      return (
                        <tr key={s.id}>
                          ...
                          <td>{s.seller_name || "—"}</td>
                          <td>
                            <code className="seller-code-inline">{s.seller_code || "—"}</code>
                          </td>
                          <td>{s.buyer_name || "—"}</td>
                          <td>{s.quantity}</td>
                          <td>
                          ...
                          <td>
                            {s.purchase_price != null
                              ? `${formatMoney(s.purchase_price)} ${countrySymbol(s.shop_country)}`
                              : "—"}
                          </td>
                          <td>
          ...
          {payForm && (
            <div className="modal-overlay" onClick={() => setPayForm(null)}>
              <div className="modal" onClick={(e) => e.stopPropagation()}>
                <div className="modal-header">
                  <h3>
                    💸 {payForm.kind === "referral" ? t("Payer le parrain") : t("Payer le vendeur")}
                  </h3>
                  <button className="drawer-close" onClick={() => setPayForm(null)}>
                    ✕
                  </button>
                </div>
                {payForm.target === "grouped" ? (
                  <div className="deliver-recap">
                    {payForm.kind === "referral" ? (
                      <>
                        <p>
                          <strong>{t("Parrain")} :</strong> {payForm.group.parrain_name}
                        </p>
                        <p>
                          <strong>{t("Nombre de ventes")} :</strong> {payForm.group.items.length}
                        </p>
                        <p>
                          <strong>🎁 {t("Commission parrainage (2%)")} :</strong>{" "}
                          {formatMoney(payForm.group.pending)}{" "}
                          {countrySymbol(payForm.group.items[0]?.shop_country)}
                        </p>
                      </>
                    ) : (
                      <>
                        <p>
                          <strong>{t("Vendeur")} :</strong> {payForm.group.seller_name} (
                          {payForm.group.seller_code || "—"})
                        </p>
                        <p>
                          <strong>{t("Nombre de ventes")} :</strong> {payForm.group.items.length}
                        </p>
                        <p>
                          <strong>{t("Commission produit")} :</strong>{" "}
                          {formatMoney(payForm.group.pending)}{" "}
                          {countrySymbol(payForm.group.items[0]?.shop_country)}
                        </p>
                      </>
                    ...
                    <p>
                      <strong>{t("Total à payer")} :</strong> {formatMoney(payForm.group.pending)}{" "}
                      {countrySymbol(payForm.group.items[0]?.shop_country)}
                    </p>
                  </div>
                ) : (
                  <div className="deliver-recap">
                    <p>
                      <strong>{t("Article")} :</strong> {payForm.sale.product_name}
                    </p>
                    {payForm.kind === "referral" ? (
                      <>
                        <p>
                          <strong>{t("Parrain")} :</strong> {payForm.sale.parrain_name || "—"}
                        </p>
                        <p>
                          <strong>🎁 {t("Commission parrainage (2%)")} :</strong>{" "}
                          {formatMoney(payForm.sale.referral_commission)}{" "}
                          {countrySymbol(payForm.sale.shop_country)}
                        </p>
                      </>
                    ) : (
                      <>
                        <p>
                          <strong>{t("Vendeur")} :</strong> {payForm.sale.seller_name} (
                          {payForm.sale.seller_code || "—"})
                        </p>
                        <p>
                          <strong>{t("Commission produit")} :</strong>{" "}
                          {formatMoney(payForm.sale.commission)}{" "}
                          {countrySymbol(payForm.sale.shop_country)}
                        </p>
                        {Number(payForm.sale.referral_commission || 0) > 0 && (
                          <p>
                            ...
                            {formatMoney(payForm.sale.referral_commission)}{" "}
                            {countrySymbol(payForm.sale.shop_country)} (
                            {t("à payer séparément au parrain")})
                          </p>
                        )}
                      </>
                    )}
                    <p>
                      <strong>{t("Total à payer")} :</strong>{" "}
                      {formatMoney(
                        payForm.kind === "referral"
                          ? Number(payForm.sale.referral_commission || 0)
                          : Number(payForm.sale.commission)
                      )}{" "}
                      {countrySymbol(payForm.sale.shop_country)}
                    </p>
                  </div>
                ...
                {payForm.methods ? (
                  <div className="deliver-recap">
                    <p>
                      <strong>
                        {payForm.kind === "referral"
                          ? t("Moyens de paiement du parrain")
                          : t("Moyens de paiement du vendeur")}
                      </strong>
                    </p>
                    {payForm.methods.full_name ? (
                      <p>
                        {t("Nom")} : {payForm.methods.full_name}
                      </p>
                    ) : null}
                    {payForm.methods.wallets.length === 0 ? (
                      <p className="hint">
                        {t("Le vendeur n'a pas encore enregistré de moyen de paiement.")}
                      </p>
                    ) : (
                      payForm.methods.wallets.map((w) => (
                        <p key={w.name}>
                          💳 {w.name} : <strong>{w.value}</strong>
                        </p>
                      ))
                    )}
                  </div>
