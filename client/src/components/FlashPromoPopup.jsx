  ...
  useEffect(() => {
    ...
    const load = () => {
      api
        ...
        .then((d) => {
          if (!ok) return;
          const list = (d.promotions || []).filter((p) => !isDismissed(p.id)).slice(0, MAX_DISPLAY);
          setPromos(list);
        })
        .catch(() => {});
    };
      ...
      {shown.map((p, i) => {
        ...
        return (
          ...
          >
            ...
            <Link
              to={`/produit/${p.product_id}`}
              className="flash-popup-body"
              tabIndex={depth > 0 ? -1 : undefined}
            >
              {p.image && (
              ...
              <div className="flash-popup-info">
                ...
                <p className="flash-popup-price">
                  ...
                  <span className="price price-flash">
                    {formatMoney(p.promo_price)} {symbol}
                  </span>
                ...
                <FlashCountdown
                  ...
                  render={({ remains, ended }) =>
                    ended ? (
                      ...
                      <span className="flash-countdown flash-countdown-big">
                        ⏰ {formatFlashTime(remains)}
                      </span>
                    )
                  }
                />
