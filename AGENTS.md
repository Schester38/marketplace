...
- **Client** : React 18 + Vite 5 (`client/`, dev 5173, proxy `/api` → `localhost:4000`), version `1.57.57`.
...
- **PWA** : `client/public/sw.js`, cache `mboppi-v288`, app shell + API_SWR + push + 4 manifests.
...
(Changelog partiel — version courante **1.57.57** / cache PWA **mboppi-v288**. Les entrées ci-dessous s'arrêtent à 1.52.2 ; les versions 1.57.x sont résumées en fin de liste.)
